...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      ...
      className={cn(
        "group/animated-card relative w-[380px] overflow-hidden rounded-2xl border border-border/50 glass-card shadow-elevated backdrop-blur-sm transition-all duration-500 hover:shadow-elevated hover:scale-105",
        className
      )}
      {...props}
    />
...
export function CardBody({ className, ...props }: CardProps) {
  return (
    <div
      role="group"
      className={cn(
        "flex flex-col space-y-2 border-t border-border/30 p-6",
        className
      )}
      {...props}
    />
...
export function CardTitle({ className, ...props }: CardTitleProps) {
  return (
    <h3
      className={cn(
        "text-xl leading-none font-bold tracking-tight text-foreground",
        className
      )}
      {...props}
    />
...
const AnalyticsVisual: React.FC<VisualProps> = ({
  ...
  secondaryColor = "hsl(var(--accent-luxury))",
    ...
    <>
      ...
      <div className="relative h-[200px] w-[380px] overflow-hidden rounded-t-2xl">
        ...
        <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[6] flex w-[380px] translate-y-0 items-start justify-center bg-transparent p-6 transition-transform duration-700 group-hover/animated-card:translate-y-full">
          <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] rounded-lg border border-border/40 bg-card/40 px-4 py-3 opacity-100 backdrop-blur-md transition-opacity duration-500 group-hover/animated-card:opacity-0">
        ...
        <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[7] flex items-center justify-center opacity-0 transition-opacity duration-700 group-hover/animated-card:opacity-100">
          {techItems.map((item, index) => (
            <div
              key={item.id}
              className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute flex items-center justify-center gap-2 rounded-full border border-border/60 bg-card/80 px-3 py-1.5 backdrop-blur-md transition-all duration-700"
              style={{
                transform: hovered
    ...
    <>
      ...
      <div className="relative h-[200px] w-[380px] overflow-hidden rounded-t-2xl">
        ...
        <div className="ease-[cubic-bezier(0.6, 0.6, 0, 1)] absolute inset-0 z-[8] flex items-center justify-center transition-transform duration-700 group-hover/animated-card:scale-110 group-hover/animated-card:rotate-12">
          <div className="flex h-16 w-16 items-center justify-center rounded-full bg-card/80 backdrop-blur-sm">
...
const GeometricVisual: React.FC<VisualProps> = ({
  mainColor = "hsl(var(--accent-luxury))",
  secondaryColor = "hsl(var(--accent-error))",
  gridColor = "hsl(var(--border))",
}) => {
  const [hovered, setHovered] = useState(false)
...
const NetworkVisual: React.FC<VisualProps> = ({
  mainColor = "hsl(var(--accent-success))",
